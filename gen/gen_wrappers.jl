...
append!(bound_api["H5T"], ["h5t_get_member_name",   # defined in src/api/helpers.jl
...
open(joinpath(@__DIR__, "..", "src", "api", "functions.jl"), "w") do fid
    ...
    ismacro(ex, sym, n=0) = isexpr(ex, :macrocall) && length(ex.args) >= n + 2 && ex.args[1] == sym
    for funcblock in exprs.args
        ...
        elseif ismacro(funcblock, Symbol("@static"), 1) &&
            ...
            buf = replace(buf,
                          r"^\s+@doc \"SENTINEL_DOC\" "m => triplequote(docstr, " "^4, "@doc ") * " "^4)
            print(fid, buf, "\n\n")
        else
            # passthrough
            print(fid, funcblock, "\n\n")
...
apidocs = ""
for (mod, desc, urltail) in (("H5", "General Library Functions", "Library"),
