...
function h5e_get_msg(mesg_id)
    ...
    buffer = Array{UInt8}(undef, mesg_len+1)
    mesg_len = h5e_get_msg(mesg_id, mesg_type, buffer, length(buffer))
...
function h5p_get_class_name(pcid)
    ...
    if pc == C_NULL
        @hdf5error("Error getting class name")
    end
    s = unsafe_string(pc)
    h5_free_memory(pc)
    return s
...
function h5t_get_member_name(type_id, index)
    ...
    if pn == C_NULL
        @hdf5error("Error getting name of compound datatype member #$index")
    end
    s = unsafe_string(pn)
    h5_free_memory(pn)
    return s
...
function h5t_get_tag(type_id)
    ...
    if pc == C_NULL
        @hdf5error("Error getting opaque tag")
    end
    s = unsafe_string(pc)
    h5_free_memory(pc)
    return s
