...
h5open(fn, "w", comm, info) do f
    ...
    dset = create_dataset(g, "B", datatype(Int64), dataspace(10, nprocs), chunk=(10, 1), dxpl_mpio=HDF5.API.H5FD_MPIO_COLLECTIVE)
    dset[:, myrank + 1] = A
end
...
h5open(fn, comm) do f  # default: opened in read mode, with default MPI.Info()
    ...
    B = read(f, "mygroup/B", dxpl_mpio=HDF5.API.H5FD_MPIO_COLLECTIVE)
    @test !isempty(B)
    @test A == vec(B[:, myrank + 1])

    B = f["mygroup/B", dxpl_mpio=HDF5.API.H5FD_MPIO_COLLECTIVE]
    @test !isempty(B)
    @test A == vec(B[:, myrank + 1])
end
...
B = h5read(fn, "mygroup/B", fapl_mpio=(comm, info), dxpl_mpio=HDF5.API.H5FD_MPIO_COLLECTIVE)
@test A == vec(B[:, myrank + 1])

MPI.Barrier(comm)

B = h5read(fn, "mygroup/B", (:, myrank + 1), fapl_mpio=(comm, info), dxpl_mpio=HDF5.API.H5FD_MPIO_COLLECTIVE)
@test A == vec(B)
