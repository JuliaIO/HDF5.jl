...
@testset "h5d_gather" begin
    ...
    h5open(fn ,"w") do f
        ...
        @test_throws HDF5.HDF5Error HDF5.h5d_gather(dataspace(d), src_buf, datatype(Int), sizeof(dst_buf)÷2, dst_buf, gatherf_bad_ptr, C_NULL)
        ...
        @test_throws HDF5.HDF5Error HDF5.h5d_gather(dataspace(d), src_buf, datatype(Int), sizeof(dst_buf)÷2, dst_buf, gatherf_data_ptr, 10)
    end
    rm(fn)
end
...
@testset "h5d_scatter" begin
    h5open(fn, "w") do f
        ...
        @test_throws HDF5.HDF5Error HDF5.h5d_scatter(scatterf_bad_ptr, C_NULL, datatype(Int), dataspace(d), dst_buf)
